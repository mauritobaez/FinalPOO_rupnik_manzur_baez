...
public class Ellipse extends Figure
{
    ...
    {
        double p = (Math.pow(centerPoint.getXDifferenceTo(point), 2) / Math.pow(sXAxis, 2))
                + (Math.pow(centerPoint.getYDifferenceTo(point), 2) / Math.pow(sYAxis, 2));
        return Double.compare(p,1.0) < 0;
    }
}
